...
void * receive_info (void *arg)
{
	...
	// initializing info from receiving packet
	int srcID = ntohl(packet[0]);
	int destID = ntohl(packet[1]);
	int cost = ntohl(packet[2]);

	// updating costs table
	pthread_mutex_lock(&lock);
	costs[srcID][destID] = cost;
	costs[destID][srcID] = cost;
...
void * run_link_state (void *arg)
{
	...
	while (1)
	{
		...
		for (i = 1; i < N; i++)
		{
			// find closest node (used Dijksta's algorithm)
			min = INFINITE;
			spot = -1;
